# Release Process for Auto-PR

This document outlines the process for releasing new versions of Auto-PR to PyPI using automated GitHub Actions.

## Overview

Auto-PR uses an automated release process. When you push a version tag to GitHub, a GitHub Actions workflow automatically builds and publishes the package to PyPI. **No manual uploading is required.**
...
Update version in `src/auto_pr/__version__.py`:
...
- Interactive language selection with `auto-pr language`
...
git add src/auto_pr/__version__.py CHANGELOG.md
...
2. Workflow verifies the tag version matches `src/auto_pr/__version__.py`
...
Monitor the [Actions tab](https://github.com/cellwebb/auto-pr/actions) to track the release progress.

### 6. Post-release Verification

1. **Check GitHub Actions**:

   - Go to [Actions tab](https://github.com/cellwebb/auto-pr/actions)
   - Verify the "Publish to PyPI" workflow succeeded
   - Check for any errors in the logs

2. **Verify on PyPI**:

   - Check [PyPI project page](https://pypi.org/project/auto-pr/)
...
3. **Test Installation**:
   ...
   uv tool install --reinstall auto-pr

   # Verify version
   auto-pr --version  # Should show the new version
   ```

4. **Create GitHub Release** (optional but recommended):

   - Go to [Releases](https://github.com/cellwebb/auto-pr/releases)
...
3. **Verifies** tag version matches `src/auto_pr/__version__.py`
...
1. Go to [Actions tab](https://github.com/cellwebb/auto-pr/actions)
2. Click on the failed workflow run
3. Check which step failed

**Common issues:**

- **Version mismatch**: Tag version doesn't match `src/auto_pr/__version__.py`
...
1. **Yank on PyPI**:
   ...
   - Go to [PyPI project page](https://pypi.org/project/auto-pr/)
   - Select the version
...
# Monitor release
open https://github.com/cellwebb/auto-pr/actions
...
- Check [GitHub Actions logs](https://github.com/cellwebb/auto-pr/actions)
