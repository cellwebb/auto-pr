# Using Claude Code with Auto-PR
...
Auto-PR supports authentication via Claude Code subscriptions, allowing you to use your Claude Code subscription instead of paying for the expensive Anthropic API. This is perfect for users who already have Claude Code access through their subscription.
...
Auto-PR includes built-in OAuth authentication for Claude Code. The setup process is fully automated and will open your browser for authentication.

### Option 1: During Initial Setup (Recommended)

When running `auto-pr init`, simply select "Claude Code" as your provider:
...
3. Save your access token to `~/.auto-pr.env`
4. Set the default model

### Option 2: Switch to Claude Code Later

If you already have Auto-PR configured with another provider and want to switch to Claude Code:
...
auto-pr -m claude-code:claude-sonnet-4-5
```
...
```bash
auto-pr auth claude-code login
```

Your browser will open automatically for fresh OAuth authentication. Alternatively, you can run `auto-pr model`, select "Claude Code (OAuth)", and choose "Re-authenticate (get new token)".
...
```bash
auto-pr auth claude-code status
```
...
```bash
auto-pr auth claude-code logout
```
...
| Setup            | Manual API key entry          | Automatic OAuth via `auto-pr init` or `auto-pr model` |
...
- Auto-PR automatically stores tokens in `~/.auto-pr.env` (outside your project directory)
- Tokens may expire and will require re-authentication via `auto-pr model`
