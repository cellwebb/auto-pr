...
This guide explains how to customize the system prompt that Auto-PR uses to generate commit messages, allowing you to define your own commit message style and conventions.
...
Auto-PR uses two prompts when generating commit messages:
...
1. **Create your custom system prompt file:**
   ...
   cp custom_system_prompt.example.txt ~/.config/auto-pr/my_system_prompt.txt

   # Or create your own from scratch
   vim ~/.config/auto-pr/my_system_prompt.txt
   ```

2. **Add to your `.auto-pr.env` file:**

   ```bash
   # In ~/.auto-pr.env or project-level .auto-pr.env
   AUTO_PR_SYSTEM_PROMPT_PATH=/path/to/your/custom_system_prompt.txt
   ```
...
That's it! Auto-PR will now use your custom instructions instead of the default.
...
- **Project-specific prompts** - Use project-level `.auto-pr.env` for project-specific styles
...
**Solution:** This shouldn't happen—Auto-PR automatically disables conventional commit enforcement when using custom system prompts. If you see this, please [file an issue](https://github.com/cellwebb/auto-pr/issues).
...
**Problem:** Auto-PR still uses default commit format.

**Solution:**

1. Check that `AUTO_PR_SYSTEM_PROMPT_PATH` is set correctly:

   ```bash
   auto-pr config get AUTO_PR_SYSTEM_PROMPT_PATH
   ```

2. Verify the file path exists and is readable:

   ```bash
   cat "$AUTO_PR_SYSTEM_PROMPT_PATH"
   ```

3. Check `.auto-pr.env` files in this order:
   - Project level: `./.auto-pr.env`
   - User level: `~/.auto-pr.env`
...
This will present an interactive menu with 25+ languages in their native scripts (Español, Français, 日本語, etc.). Select your preferred language, and it will automatically set `AUTO_PR_LANGUAGE` in your `~/.auto-pr.env` file.
...
**Want to translate prefixes too?** Set `AUTO_PR_TRANSLATE_PREFIXES=true` in your `.auto-pr.env` for full localization:
...
auto-pr config unset AUTO_PR_SYSTEM_PROMPT_PATH

# Option 2: Comment it out in .auto-pr.env
# AUTO_PR_SYSTEM_PROMPT_PATH=/path/to/custom_prompt.txt

# Option 3: Use a different .auto-pr.env for specific projects
```
...
- Report issues: [GitHub Issues](https://github.com/cellwebb/auto-pr/issues)
