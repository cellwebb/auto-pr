...
This guide covers common issues and solutions for installing, configuring, and running auto-pr.

## Table of Contents

- [Troubleshooting auto-pr](#troubleshooting-auto-pr)
...
**Problem:** `auto-pr` command not found after install

- Ensure you installed with `uv tool install auto-pr` or `uvx auto-pr`
...
**Problem:** auto-pr can't find your API key or model

- If you are new, run `auto-pr init` to interactively set up your provider, model, and API keys
- Make sure your `.auto-pr.env` or environment variables are set correctly
- Run `auto-pr --log-level=debug` to see which config files are loaded and debug configuration issues
- Check for typos in variable names (e.g., `AUTO_PR_GROQ_API_KEY`)

**Problem:** User-level `$HOME/.auto-pr.env` changes are not picked up

- Make sure you are editing the correct file for your OS:
  - On macOS/Linux: `$HOME/.auto-pr.env` (usually `/Users/<your-username>/.auto-pr.env` or `/home/<your-username>/.auto-pr.env`)
  - On Windows: `$HOME/.auto-pr.env` (typically `C:\Users\<your-username>\.auto-pr.env` or use `%USERPROFILE%`)
- Run `auto-pr --log-level=debug` to confirm the user-level config is loaded
...
**Problem:** Project-level `.auto-pr.env` changes are not picked up

- Ensure your project contains a `.auto-pr.env` file in the root directory (next to your `.git` folder)
- Run `auto-pr --log-level=debug` to confirm the project-level config is loaded
- If you edit `.auto-pr.env`, restart your terminal or re-run your shell to reload environment variables
- If still not working, check for typos and file permissions

**Problem:** Cannot set or change language for commit messages

- Run `auto-pr language` (or `auto-pr lang`) to interactively select from 25+ supported languages
- Use `-l <language>` flag to override language for a single commit (e.g., `auto-pr -l zh-CN`, `auto-pr -l Spanish`)
- Check your config with `auto-pr config show` to see current language setting
- Language setting is stored in `AUTO_PR_LANGUAGE` in your `.auto-pr.env` file
...
- **For Claude Code token expiration**: Run `auto-pr auth` to quickly re-authenticate and refresh your token. Your browser will open automatically for OAuth.
...
- Use `auto-pr --show-prompt` to debug what the LLM is seeing
...
- Set `AUTO_PR_SKIP_SECRET_SCAN=true` in your `.auto-pr.env` file
- Use `auto-pr config set AUTO_PR_SKIP_SECRET_SCAN true`
...
- Use `auto-pr --no-verify` to skip all pre-commit and lefthook hooks temporarily
...
- auto-pr requires staged changes to generate a commit message
- Use `git add <files>` to stage changes, or use `auto-pr -a` to stage all changes automatically
- Check `git status` to see what files have been modified
- Use `auto-pr diff` to see a filtered view of your changes
...
**Problem:** auto-pr is too slow

- Use `auto-pr -y` to skip the confirmation prompt
- Use `auto-pr -q` for quiet mode with less output
- Consider using faster/cheaper models for routine commits
- Use `auto-pr --no-verify` to skip hooks if they're slowing you down
...
- Use `auto-pr init` to reset or update your configuration interactively
- Use `auto-pr --log-level=debug` for detailed debug output and logging
- Use `auto-pr --show-prompt` to see what prompt is being sent to the LLM
- Use `auto-pr --help` to see all available command-line flags
- Use `auto-pr config show` to see all current configuration values
- Check logs for error messages and stack traces
- Check the main [README.md](../README.md) for features, examples, and quick start instructions

## Still Stuck?

- Search existing issues or open a new one on the [GitHub repository](https://github.com/cellwebb/auto-pr)
- Include details about your OS, Python version, auto-pr version, provider, and error output
