...
from auto_pr.constants import MessageConstants
...
def _remove_think_tags(message: str) -> str:
    ...
    conventional_prefixes_pattern = r"(" + "|".join(MessageConstants.CONVENTIONAL_PREFIXES) + r")[\(:)]"
...
def _extract_commit_from_reasoning(message: str) -> str:
    ...
    """
    for indicator in MessageConstants.MESSAGE_INDICATORS:
    ...
    for i, line in enumerate(lines):
        if any(line.strip().startswith(f"{prefix}:") for prefix in MessageConstants.CONVENTIONAL_PREFIXES):
            message = "\n".join(lines[i:])
            break
...
def _remove_xml_tags(message: str) -> str:
    ...
    """
    for tag in MessageConstants.XML_TAGS_TO_REMOVE:
...
def _fix_double_prefix(message: str) -> str:
    ...
    double_prefix_pattern = re.compile(
        r"^("
        + r"|\s*".join(MessageConstants.CONVENTIONAL_PREFIXES)
        + r"):\s*("
        + r"|\s*".join(MessageConstants.CONVENTIONAL_PREFIXES)
        + r")\(([^)]+)\):"
    )
