"""Error handling module for auto-pr."""
...
class AutoPRError(Exception):
    """Base exception class for all auto-pr errors."""
...
class FormattingError(AutoPRError):
    """Error related to code formatting."""

    exit_code = 5


class SecurityError(AutoPRError):
    """Error related to security issues (e.g., detected secrets)."""

    exit_code = 6


class HookError(AutoPRError):
...
def with_error_handling(
    error_type: type[AutoPRError], error_message: str, quiet: bool = False, exit_on_error: bool = True
