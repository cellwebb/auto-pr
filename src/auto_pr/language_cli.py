...
from auto_pr.constants import Languages
...
def configure_language_init_workflow(existing_env_path: Path | str) -> bool:
    ...
    """
    try:
        # Use the provided path instead of our default AUTO_PR_ENV_PATH
        ...
        env_keys_to_clear = [k for k in os.environ.keys() if k.startswith("AUTO_PR_")]
        for key in env_keys_to_clear:
            del os.environ[key]

        # File exists - check for existing language
        load_dotenv(temp_env_path)
        existing_language = os.getenv("AUTO_PR_LANGUAGE")
...
def _run_language_selection_flow(env_path: Path) -> str | None:
    ...
    if language_selection == "English":
        set_key(str(env_path), "AUTO_PR_LANGUAGE", "English")
        set_key(str(env_path), "AUTO_PR_TRANSLATE_PREFIXES", "false")
        click.echo("Set AUTO_PR_LANGUAGE='English'")
        click.echo("Set AUTO_PR_TRANSLATE_PREFIXES='false'")
        return "English"
    ...
    set_key(str(env_path), "AUTO_PR_LANGUAGE", language_value)
    set_key(str(env_path), "AUTO_PR_TRANSLATE_PREFIXES", "true" if translate_prefixes else "false")
    click.echo(f"Set AUTO_PR_LANGUAGE='{language_value}'")
    click.echo(f"Set AUTO_PR_TRANSLATE_PREFIXES={'true' if translate_prefixes else 'false'}")
...
def show_rtl_warning(language_name: str, env_path: Path | None = None) -> bool:
    ...
    Args:
        ...
        env_path: Path to environment file (defaults to AUTO_PR_ENV_PATH)

    Returns:
        True if user wants to proceed, False if they cancel
    """
    if env_path is None:
        env_path = AUTO_PR_ENV_PATH
    ...
    click.echo()
    click.echo("Right-to-left (RTL) languages may not display correctly in auto-pr due to terminal limitations.")
    ...
    if proceed:
        ...
        set_key(str(env_path), "AUTO_PR_RTL_CONFIRMED", "true")
...
@click.command()
def language() -> None:
    """Set the language for PR descriptions interactively."""
    click.echo("Select a language for your commit messages:\n")

    # Ensure .auto-pr.env exists
    if not AUTO_PR_ENV_PATH.exists():
        AUTO_PR_ENV_PATH.touch()
        click.echo(f"Created {AUTO_PR_ENV_PATH}")

    language_value = _run_language_selection_flow(AUTO_PR_ENV_PATH)
    ...
    click.echo(f"  AUTO_PR_LANGUAGE={language_value}")
    ...
    if translate_prefixes:
        click.echo("  AUTO_PR_TRANSLATE_PREFIXES=true")
        click.echo("\n  Prefixes will be translated (e.g., 'corrección:' instead of 'fix:')")
    else:
        click.echo("  AUTO_PR_TRANSLATE_PREFIXES=false")
        click.echo(f"\n  Prefixes will remain in English (e.g., 'fix: <{language_value} description>')")

    click.echo(f"\n  Configuration saved to {AUTO_PR_ENV_PATH}")
