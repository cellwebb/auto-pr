"""Utilities for AI provider integration for auto-pr.
...
from auto_pr.constants import EnvDefaults, Utility
from auto_pr.errors import AIError
from auto_pr.oauth import QwenOAuthProvider, refresh_token_if_expired
from auto_pr.oauth.token_store import TokenStore
from auto_pr.providers import SUPPORTED_PROVIDERS
...
def _should_skip_tiktoken_counting() -> bool:
    ...
    value = os.getenv("AUTO_PR_NO_TIKTOKEN", str(EnvDefaults.NO_TIKTOKEN))
    ...
    if provider == "claude-code":
        ...
        if not refresh_token_if_expired(quiet=True):
            raise AIError.authentication_error(
                "Claude Code token not found or expired. Please authenticate with 'auto-pr auth claude-code login'."
            )
        ...
        if token_data and "access_token" in token_data:
            ...
            raise AIError.authentication_error(
                "Claude Code token not found. Please authenticate with 'auto-pr auth claude-code login'."
            )
    ...
    if provider == "qwen":
        ...
        if not token:
            ...
            try:
                ...
                if not token:
                    raise AIError.authentication_error(
                        "Qwen authentication failed. Run 'auto-pr auth qwen login' to authenticate manually."
                    )
                if not quiet:
                ...
                raise AIError.authentication_error(
                    f"Qwen authentication failed: {e}. Run 'auto-pr auth qwen login' to authenticate manually."
                ) from e
