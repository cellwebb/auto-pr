...
def auth(ctx: click.Context) -> None:
    ...
    Examples:
        auto-pr auth                        # Show authentication status
        auto-pr auth claude-code login      # Login to Claude Code
        auto-pr auth claude-code logout     # Logout from Claude Code
        auto-pr auth claude-code status     # Check Claude Code auth status
        auto-pr auth qwen login             # Login to Qwen
        auto-pr auth qwen logout            # Logout from Qwen
        auto-pr auth qwen status            # Check Qwen auth status
    """
...
def _show_auth_status() -> None:
    ...
    if claude_token:
        ...
        click.echo("             Run 'auto-pr auth claude-code login' to login")
    ...
    if qwen_token:
        ...
        click.echo("             Run 'auto-pr auth qwen login' to login")
...
def claude_code_login(quiet: bool = False) -> None:
    ...
    securely in ~/.auto-pr/oauth/claude-code.json.
    """
    if not quiet:
        setup_logging("INFO")
...
def claude_code_status() -> None:
    ...
    if token:
        ...
        click.echo("Run 'auto-pr auth claude-code login' to authenticate.")
...
def qwen_login(no_browser: bool = False, quiet: bool = False) -> None:
    ...
    securely in ~/.auto-pr/oauth/qwen.json.
    """
    if not quiet:
        setup_logging("INFO")
...
def qwen_status() -> None:
    ...
    if token:
        ...
        click.echo("Run 'auto-pr auth qwen login' to authenticate.")
