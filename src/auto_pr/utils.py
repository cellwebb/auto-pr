...
def should_skip_ssl_verification() -> bool:
    ...
    Can be enabled via:
    - AUTO_PR_NO_VERIFY_SSL=true environment variable
    - --no-verify-ssl CLI flag (which sets the env var)

    Returns:
        True if SSL verification should be skipped, False otherwise.
    """
    value = os.getenv("AUTO_PR_NO_VERIFY_SSL", str(EnvDefaults.NO_VERIFY_SSL))
...
def run_subprocess_with_encoding(
    ...
    Raises:
        AutoPRError: If the command times out
    ...
    try:
        ...
        raise AutoPRError(f"Command timed out: {' '.join(command)}") from e
...
def run_subprocess(
    ...
    for encoding in encodings:
        try:
            ...
            continue
        except (subprocess.CalledProcessError, AutoPRError, subprocess.TimeoutExpired):
