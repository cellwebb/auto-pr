...
def _load_default_system_template() -> str:
    ...
    return files("auto_pr.templates").joinpath("system_prompt.txt").read_text(encoding="utf-8")


@lru_cache(maxsize=1)
def _load_default_user_template() -> str:
    """Load the default user prompt template from package resources."""
    return files("auto_pr.templates").joinpath("user_prompt.txt").read_text(encoding="utf-8")


@lru_cache(maxsize=1)
def _load_question_generation_template() -> str:
    """Load the question generation template from package resources."""
    return files("auto_pr.templates").joinpath("question_generation.txt").read_text(encoding="utf-8")
