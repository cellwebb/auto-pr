"""Configuration loading for auto-pr.

Handles environment variable and .auto-pr.env file precedence for application settings.
"""
...
from auto_pr.constants import EnvDefaults, Logging
from auto_pr.errors import ConfigError


class AutoPRConfig(TypedDict, total=False):
    """TypedDict for Auto-PR configuration values.
...
def load_config() -> AutoPRConfig:
    """Load configuration from $HOME/.auto-pr.env, then ./.auto-pr.env, then environment variables."""
    user_config = Path.home() / ".auto-pr.env"
    if user_config.exists():
        load_dotenv(user_config)

    # Check for .auto-pr.env in project directory
    project_auto_pr_env = Path(".auto-pr.env")
    ...
    config: AutoPRConfig = {
        ...
        "max_output_tokens": int(os.getenv("AUTO_PR_MAX_OUTPUT_TOKENS", EnvDefaults.MAX_OUTPUT_TOKENS)),
        "max_retries": int(os.getenv("AUTO_PR_RETRIES", EnvDefaults.MAX_RETRIES)),
        "log_level": os.getenv("AUTO_PR_LOG_LEVEL", Logging.DEFAULT_LEVEL),
        "warning_limit_tokens": int(os.getenv("AUTO_PR_WARNING_LIMIT_TOKENS", EnvDefaults.WARNING_LIMIT_TOKENS)),
        "always_include_scope": os.getenv("AUTO_PR_ALWAYS_INCLUDE_SCOPE", str(EnvDefaults.ALWAYS_INCLUDE_SCOPE)).lower()
        in ("true", "1", "yes", "on"),
        "skip_secret_scan": os.getenv("AUTO_PR_SKIP_SECRET_SCAN", str(EnvDefaults.SKIP_SECRET_SCAN)).lower()
        in ("true", "1", "yes", "on"),
        "no_tiktoken": os.getenv("AUTO_PR_NO_TIKTOKEN", str(EnvDefaults.NO_TIKTOKEN)).lower()
        in ("true", "1", "yes", "on"),
        "no_verify_ssl": os.getenv("AUTO_PR_NO_VERIFY_SSL", str(EnvDefaults.NO_VERIFY_SSL)).lower()
        in ("true", "1", "yes", "on"),
        "verbose": os.getenv("AUTO_PR_VERBOSE", str(EnvDefaults.VERBOSE)).lower() in ("true", "1", "yes", "on"),
        "system_prompt_path": os.getenv("AUTO_PR_SYSTEM_PROMPT_PATH"),
        "language": os.getenv("AUTO_PR_LANGUAGE"),
