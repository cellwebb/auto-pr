"""CLI for managing auto-pr model configuration in $HOME/.auto-pr.env."""
...
def _should_show_rtl_warning_for_init() -> bool:
    """Check if RTL warning should be shown based on init's AUTO_PR_ENV_PATH.
...
def _show_rtl_warning_for_init(language_name: str) -> bool:
    """Show RTL language warning for init command and save preference to AUTO_PR_ENV_PATH.
    ...
    if proceed:
        ...
        set_key(str(AUTO_PR_ENV_PATH), "AUTO_PR_RTL_CONFIRMED", "true")
        return True
    else:
...
def _configure_model(existing_env: dict[str, str]) -> bool:
    ...
    set_key(str(AUTO_PR_ENV_PATH), "AUTO_PR_MODEL", f"{provider_key}:{model_to_save}")
    click.echo(f"Set AUTO_PR_MODEL={provider_key}:{model_to_save}")
    ...
    if is_custom_anthropic:
        ...
        if base_url is None:
            ...
            return False
        set_key(str(AUTO_PR_ENV_PATH), "CUSTOM_ANTHROPIC_BASE_URL", base_url)
        ...
        if api_version and api_version != "2023-06-01":
            set_key(str(AUTO_PR_ENV_PATH), "CUSTOM_ANTHROPIC_VERSION", api_version)
        ...
        if existing_endpoint:
            ...
            if endpoint_action == "Enter new endpoint":
                ...
                if endpoint is None:
                    ...
                    return False
                set_key(str(AUTO_PR_ENV_PATH), "AZURE_OPENAI_ENDPOINT", endpoint)
        ...
        if existing_api_version:
            ...
            if version_action == "Enter new version":
                ...
                set_key(str(AUTO_PR_ENV_PATH), "AZURE_OPENAI_API_VERSION", api_version)
        ...
        if base_url is None:
            ...
            return False
        set_key(str(AUTO_PR_ENV_PATH), "CUSTOM_OPENAI_BASE_URL", base_url)
        ...
        set_key(str(AUTO_PR_ENV_PATH), "OLLAMA_API_URL", url_to_save)
        ...
        if url is None:
            ...
            return False
        url_to_save = url.strip() if url.strip() else url_default
        set_key(str(AUTO_PR_ENV_PATH), "LMSTUDIO_API_URL", url_to_save)
    ...
    if is_claude_code:
        ...
        from auto_pr.oauth.token_store import TokenStore
    ...
    if existing_key:
        ...
        if action is None:
            ...
            if api_key and api_key.strip():
                set_key(str(AUTO_PR_ENV_PATH), api_key_name, api_key)
        ...
        if api_key and api_key.strip():
            ...
            click.echo("No API key entered. You can add one later by editing ~/.auto-pr.env")
...
def model() -> None:
    ...
    click.echo("Welcome to Auto-PR model configuration!\n")

    existing_env = _load_existing_env()
    if not _configure_model(existing_env):
        return

    click.echo(f"\nModel configuration complete. You can edit {AUTO_PR_ENV_PATH} to update values later.")
