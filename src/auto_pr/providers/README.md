...
This directory contains AI provider implementations for Auto-PR (Git Auto Commit). All providers follow a class-based architecture with shared base classes to reduce code duplication and improve maintainability.
...
from auto_pr.providers.error_handler import handle_provider_errors
...
```python
# In src/auto_pr/providers/__init__.py
...
class TestProviderMocked(BaseProviderTest):
    ...
    @property
    def provider_module(self) -> str: return "auto_pr.providers.my_provider"
...
All HTTP requests use Auto-PR's SSL verification settings via `get_ssl_verify()`:

```python
from auto_pr.utils import get_ssl_verify
...
```python
from auto_pr.constants import ProviderDefaults
...
5. Run `mypy` for type checking: `uv run -- mypy src/auto_pr`
