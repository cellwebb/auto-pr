...
from auto_pr.ai import generate_commit_message, generate_grouped_commits
from auto_pr.ai_utils import (
    count_tokens,
    extract_text_content,
    get_encoding,
)
from auto_pr.errors import AIError
from auto_pr.providers import PROVIDER_REGISTRY, SUPPORTED_PROVIDERS
...
class TestAiUtils:
    ...
    @patch("auto_pr.ai_utils.count_tokens")
    ...
    def test_count_tokens_error_handling(self):
        ...
        with patch("auto_pr.ai_utils.get_encoding") as mock_encoding:
    ...
    def test_count_tokens_with_various_content_types(self):
        ...
        with patch("auto_pr.ai_utils.get_encoding", return_value=mock_encoding):
...
class TestGenerateCommitMessage:
    ...
    @patch.dict("auto_pr.providers.PROVIDER_REGISTRY", {"openai": MagicMock(return_value="feat: Add new feature")})
    ...
    @patch.dict("auto_pr.providers.PROVIDER_REGISTRY", {"anthropic": MagicMock(return_value="fix: Resolve bug")})
    ...
    @patch("auto_pr.ai_utils.Status")
    @patch("auto_pr.ai_utils.console")
    @patch.dict("auto_pr.providers.PROVIDER_REGISTRY", {"openai": MagicMock(return_value="docs: Update README")})
    ...
    @patch.dict("auto_pr.providers.PROVIDER_REGISTRY", {"openrouter": MagicMock(return_value="chore: tidy config")})
    ...
    @patch.dict(
        "auto_pr.providers.PROVIDER_REGISTRY", {"streamlake": MagicMock(return_value="feat: summarize planets")}
    )
    ...
    @patch("auto_pr.ai_utils.time.sleep")
    @patch.dict(
        "auto_pr.providers.PROVIDER_REGISTRY",
        {
            "openai": MagicMock(
                side_effect=[
    ...
    @patch.dict(
        "auto_pr.providers.PROVIDER_REGISTRY", {"openai": MagicMock(return_value="Alternative response format")}
    )
    ...
    @patch.dict(
        "auto_pr.providers.PROVIDER_REGISTRY", {"anthropic": MagicMock(return_value="feat: Add conversation support")}
    )
    ...
    @patch("auto_pr.ai.generate_with_retries")
    ...
    @patch.dict("auto_pr.providers.PROVIDER_REGISTRY", {"openai": MagicMock(return_value='{"commits": []}')})
