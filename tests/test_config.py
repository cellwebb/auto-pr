...
from auto_pr.config import load_config, validate_config
...
def test_load_config_env(tmp_path, monkeypatch):
    ...
    with patch("auto_pr.config.Path.home") as mock_home:
        mock_home.return_value = tmp_path / "nonexistent_home"

        monkeypatch.setenv("AUTO_PR_MODEL", "env-model")
        monkeypatch.setenv("AUTO_PR_TEMPERATURE", "0.5")
        monkeypatch.setenv("AUTO_PR_MAX_OUTPUT_TOKENS", "1234")
        monkeypatch.setenv("AUTO_PR_RETRIES", "7")
        monkeypatch.setenv("AUTO_PR_LOG_LEVEL", "DEBUG")
        monkeypatch.setenv("AUTO_PR_NO_TIKTOKEN", "true")
        config = load_config()
...
def test_load_config_project_auto_pr_env(tmp_path, monkeypatch):
    """Test that project .auto-pr.env file is loaded and overrides user config."""
    # Change to tmp directory
    monkeypatch.chdir(tmp_path)

    # Create a .auto-pr.env file in the project directory
    auto_pr_env = tmp_path / ".auto-pr.env"
    auto_pr_env.write_text("AUTO_PR_MODEL=project-model\nAUTO_PR_TEMPERATURE=0.8\n")

    # Mock home directory config
    with patch("auto_pr.config.Path.home") as mock_home:
        mock_home.return_value = tmp_path / "home"
        home_config = mock_home.return_value / ".auto-pr.env"
        home_config.parent.mkdir(exist_ok=True)
        home_config.write_text("AUTO_PR_MODEL=home-model\nAUTO_PR_TEMPERATURE=0.3\n")
...
def test_load_config_ignores_plain_env_file(tmp_path, monkeypatch):
    ...
    env_file.write_text("AUTO_PR_MODEL=env-file-model\n")

    with patch("auto_pr.config.Path.home") as mock_home:
        mock_home.return_value = tmp_path / "nonexistent_home"
        monkeypatch.delenv("AUTO_PR_MODEL", raising=False)
...
def test_load_config_verbose(tmp_path, monkeypatch):
    """Test that AUTO_PR_VERBOSE config option works correctly."""
    # Change to tmp directory
    monkeypatch.chdir(tmp_path)
    ...
    with patch("auto_pr.config.Path.home") as mock_home:
        ...
        monkeypatch.setenv("AUTO_PR_VERBOSE", "true")
        config = load_config()
        assert config["verbose"] is True

        # Test with verbose=false
        monkeypatch.setenv("AUTO_PR_VERBOSE", "false")
        config = load_config()
        assert config["verbose"] is False

        # Test with verbose=1
        monkeypatch.setenv("AUTO_PR_VERBOSE", "1")
        config = load_config()
        assert config["verbose"] is True

        # Test with verbose=yes
        monkeypatch.setenv("AUTO_PR_VERBOSE", "yes")
        config = load_config()
        assert config["verbose"] is True
...
class TestValidateConfig:
    ...
    def test_load_config_validates(self, tmp_path, monkeypatch):
        ...
        # Create a .auto-pr.env file with invalid temperature
        auto_pr_env = tmp_path / ".auto-pr.env"
        auto_pr_env.write_text("AUTO_PR_TEMPERATURE=3.0\n")  # Invalid: > 2.0
