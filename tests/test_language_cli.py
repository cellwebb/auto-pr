...
from auto_pr.language_cli import language
...
def test_language_select_predefined_with_prefix_translation(clean_env_state):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        with patch("auto_pr.language_cli.AUTO_PR_ENV_PATH", fake_path):
            ...
            with patch("questionary.select") as mock_select:
                ...
                assert "AUTO_PR_LANGUAGE=Spanish" in result.output
                assert "AUTO_PR_TRANSLATE_PREFIXES=true" in result.output
                assert "Prefixes will be translated" in result.output
                assert fake_path.exists()

                # Verify the file contents
                content = fake_path.read_text()
                assert "AUTO_PR_LANGUAGE=" in content and "Spanish" in content
                assert "AUTO_PR_TRANSLATE_PREFIXES=" in content and "true" in content
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        with patch("auto_pr.language_cli.AUTO_PR_ENV_PATH", fake_path):
            with patch("questionary.select") as mock_select:
                ...
                assert "AUTO_PR_LANGUAGE=Japanese" in result.output
                assert "AUTO_PR_TRANSLATE_PREFIXES=false" in result.output
                assert "Prefixes will remain in English" in result.output
                assert fake_path.exists()

                content = fake_path.read_text()
                assert "AUTO_PR_LANGUAGE=" in content and "Japanese" in content
                assert "AUTO_PR_TRANSLATE_PREFIXES=" in content and "false" in content
...
def test_language_select_english_sets_explicitly(clean_env_state):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        # Pre-populate .auto-pr.env with a language setting
        fake_path.write_text("AUTO_PR_LANGUAGE=Spanish\nAUTO_PR_TRANSLATE_PREFIXES=true\n")
        ...
        with patch("auto_pr.language_cli.AUTO_PR_ENV_PATH", fake_path):
            with patch("questionary.select") as mock_select:
                ...
                assert "AUTO_PR_LANGUAGE=English" in result.output
                assert "AUTO_PR_TRANSLATE_PREFIXES=false" in result.output

                # Verify AUTO_PR_LANGUAGE was set to English
                content = fake_path.read_text()
                assert "AUTO_PR_LANGUAGE='English'" in content
                assert "AUTO_PR_TRANSLATE_PREFIXES='false'" in content


def test_language_select_english_file_not_exists(clean_env_state):
    """Test selecting English when .auto-pr.env doesn't exist yet."""
    runner = CliRunner()
    with tempfile.TemporaryDirectory() as tmpdir:
        fake_path = Path(tmpdir) / ".auto-pr.env"
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        with patch("auto_pr.language_cli.AUTO_PR_ENV_PATH", fake_path):
            with patch("questionary.select") as mock_select, patch("questionary.text") as mock_text:
                ...
                assert "AUTO_PR_LANGUAGE=Esperanto" in result.output
                assert fake_path.exists()

                content = fake_path.read_text()
                assert "AUTO_PR_LANGUAGE=" in content and "Esperanto" in content
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        with patch("auto_pr.language_cli.AUTO_PR_ENV_PATH", fake_path):
            with patch("questionary.select") as mock_select, patch("questionary.text") as mock_text:
                ...
                assert result.exit_code == 0
                # Should be trimmed
                assert "AUTO_PR_LANGUAGE=Klingon" in result.output
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        with patch("auto_pr.language_cli.AUTO_PR_ENV_PATH", fake_path):
            with patch("questionary.select") as mock_select, patch("questionary.text") as mock_text:
                ...
                if fake_path.exists():
                    content = fake_path.read_text()
                    assert "AUTO_PR_LANGUAGE" not in content
...
def test_language_creates_file_if_not_exists(clean_env_state):
    """Test that .auto-pr.env is created if it doesn't exist."""
    runner = CliRunner()
    with tempfile.TemporaryDirectory() as tmpdir:
        fake_path = Path(tmpdir) / ".auto-pr.env"
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with patch("auto_pr.language_cli.AUTO_PR_ENV_PATH", fake_path):
                with patch("questionary.select") as mock_select:
                    ...
                    assert f"AUTO_PR_LANGUAGE={english_name}" in result.output

                    content = fake_path.read_text()
                    assert "AUTO_PR_LANGUAGE=" in content and english_name in content
...
def test_language_existing_file_is_updated(clean_env_state):
    """Test that selecting a new language updates existing .auto-pr.env file."""
    runner = CliRunner()
    with tempfile.TemporaryDirectory() as tmpdir:
        fake_path = Path(tmpdir) / ".auto-pr.env"

        # Pre-populate with existing config
        fake_path.write_text("AUTO_PR_MODEL=anthropic:claude-3-haiku\nAUTO_PR_LANGUAGE=Spanish\n")
        ...
        with patch("auto_pr.language_cli.AUTO_PR_ENV_PATH", fake_path):
            with patch("questionary.select") as mock_select:
                ...
                assert "AUTO_PR_MODEL=anthropic:claude-3-haiku" in content
                assert "AUTO_PR_LANGUAGE=" in content and "French" in content
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        with patch("auto_pr.language_cli.AUTO_PR_ENV_PATH", fake_path):
            with patch("questionary.select") as mock_select:
                ...
                assert (
                    "Translate prefixes into Italian" in result.output
                    or "AUTO_PR_TRANSLATE_PREFIXES=true" in result.output
                )


def test_rtl_detection_function(clean_env_state):
    """Test the RTL text detection function."""
    from auto_pr.language_cli import is_rtl_text
...
def test_language_select_rtl_predefined_arabic(clean_env_state):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        with patch("auto_pr.language_cli.AUTO_PR_ENV_PATH", fake_path):
            with (
                patch("auto_pr.language_cli.show_rtl_warning") as mock_rtl_warning,
                patch("questionary.select") as mock_select,
                patch("auto_pr.language_cli.should_show_rtl_warning", return_value=True),  # Force warning to show
            ):
                ...
                assert "AUTO_PR_LANGUAGE=Arabic" in result.output
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        with patch("auto_pr.language_cli.AUTO_PR_ENV_PATH", fake_path):
            with (
                ...
                assert not fake_path.exists() or "AUTO_PR_LANGUAGE" not in fake_path.read_text()
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        with patch("auto_pr.language_cli.AUTO_PR_ENV_PATH", fake_path):
            with (
                ...
                assert "AUTO_PR_LANGUAGE=Persian" in result.output
...
def test_language_select_non_rtl_no_warning(clean_env_state):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        fake_path = Path(tmpdir) / ".auto-pr.env"
        with (
            patch("auto_pr.language_cli.AUTO_PR_ENV_PATH", fake_path),
            patch("auto_pr.language_cli.show_rtl_warning") as mock_rtl_warning,
            patch("questionary.select") as mock_select,
        ):
            mock_select.return_value.ask.side_effect = [
...
def test_language_with_existing_env_file(clean_env_state):
    """Test language selection when .auto-pr.env file already exists (lines 34-35)."""
    runner = CliRunner()
    with tempfile.TemporaryDirectory() as tmpdir:
        fake_path = Path(tmpdir) / ".auto-pr.env"
        with patch("auto_pr.language_cli.AUTO_PR_ENV_PATH", fake_path):
            # Create the env file first to test the existing file path
            fake_path.write_text("# Existing config\nAUTO_PR_EXISTING=true\n")
            ...
            with patch("questionary.select") as mock_select:
                ...
                assert result.exit_code == 0
                assert "✓ Set language to Français" in result.output
                assert "AUTO_PR_LANGUAGE=French" in result.output
                assert fake_path.exists()

                # Verify the file contents include both old and new content
                content = fake_path.read_text()
                assert "AUTO_PR_EXISTING=true" in content
                assert "AUTO_PR_LANGUAGE=French" in content or "AUTO_PR_LANGUAGE='French'" in content


def test_center_text_function(clean_env_state):
    """Test the center_text function (helps cover line 245)."""
    from auto_pr.language_cli import center_text
