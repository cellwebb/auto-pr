...
def test_get_repo_root_success(monkeypatch):
    ...
    monkeypatch.setattr("auto_pr.git.run_git_command", mock_run_command)
...
def test_get_staged_files_all():
    ...
    with patch("auto_pr.git.run_git_command") as mock_run:
...
def test_get_staged_files_existing_only():
    ...
    with patch("auto_pr.git.run_git_command") as mock_run, patch("os.path.isfile") as mock_isfile:
        mock_run.return_value = "file1.py\nfile2.md\nfile3.txt"
...
def test_get_diff_exception():
    ...
    with patch("auto_pr.git.run_git_command") as mock_run, patch("auto_pr.git.logger") as mock_logger:
...
def test_push_changes_success():
    ...
    with patch("auto_pr.git.run_git_command") as mock_run_git, patch("auto_pr.git.run_subprocess") as mock_run_sub:
...
def test_push_changes_git_error():
    ...
    with (
        patch("auto_pr.git.run_git_command") as mock_run_git,
        patch("auto_pr.git.run_subprocess") as mock_run_sub,
        patch("auto_pr.git.logger") as mock_logger,
    ):
        mock_run_git.return_value = "origin\n"  # For 'git remote'
...
def test_get_diff_staged():
    with patch("auto_pr.git.run_subprocess") as mock_run:
        mock_run.return_value = "diff output"
...
def test_run_pre_commit_hooks_pre_commit_not_installed():
    ...
    with patch("os.path.exists") as mock_exists, patch("auto_pr.git.run_subprocess") as mock_run:
...
def test_run_pre_commit_hooks_success():
    ...
    with (
        ...
        patch("auto_pr.git.run_subprocess") as mock_run,
