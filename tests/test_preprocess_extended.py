...
from auto_pr.preprocess import (
...
class TestPreprocessDiffMainFunction:
    ...
    @patch("auto_pr.preprocess.count_tokens")
    @patch("auto_pr.preprocess.split_diff_into_sections")
    @patch("auto_pr.preprocess.process_sections_parallel")
    @patch("auto_pr.preprocess.score_sections")
    @patch("auto_pr.preprocess.smart_truncate_diff")
    ...
    @patch("auto_pr.preprocess.count_tokens")
    @patch("auto_pr.preprocess.filter_binary_and_minified")
...
class TestSectionProcessing:
    ...
    @patch("auto_pr.preprocess.process_section")
...
"""
        ...
        with patch("auto_pr.preprocess.split_diff_into_sections") as mock_split:
            mock_split.return_value = [
                "diff --git a/app.js b/app.js\n+new content",
                "diff --git a/app.min.js b/app.min.js\nBinary files differ",
                "diff --git a/style.css b/style.css\n+styles",
            ]
            with patch("auto_pr.preprocess.should_filter_section") as mock_filter:
                mock_filter.side_effect = [False, True, False]
                with patch("auto_pr.preprocess.extract_filtered_file_summary") as mock_summary:
    ...
    @patch("auto_pr.preprocess.logger")
