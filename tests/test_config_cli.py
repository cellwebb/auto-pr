...
from auto_pr.config_cli import config


def test_config_cli_show_set_get_unset(monkeypatch):
    runner = CliRunner()
    # Use a temp file for $HOME/.auto-pr.env
    with tempfile.TemporaryDirectory() as tmpdir:
        monkeypatch.setenv("HOME", tmpdir)

        fake_path = Path(tmpdir) / ".auto-pr.env"
        with patch("auto_pr.config_cli.AUTO_PR_ENV_PATH", fake_path):
            # Test 'set'
...
def test_config_show_no_file():
    """Test show command when .auto-pr.env doesn't exist."""
    runner = CliRunner()
    with tempfile.TemporaryDirectory() as tmpdir:
        fake_path = Path(tmpdir) / ".auto-pr.env"
        with patch("auto_pr.config_cli.AUTO_PR_ENV_PATH", fake_path):
            result = runner.invoke(config, ["show"])
            assert result.exit_code == 0
            assert "No $HOME/.auto-pr.env found" in result.output
...
def test_config_show_both_user_and_project_exist_with_sensitivity():
    ...
    runner = CliRunner()
    with tempfile.TemporaryDirectory() as tmpdir:
        user_path = Path(tmpdir) / ".auto-pr.user.env"
        ...
        with patch("auto_pr.config_cli.AUTO_PR_ENV_PATH", user_path):
            # Mock project_env_path to point to our separate file
            with patch("auto_pr.config_cli.Path") as mock_path:
                mock_path.side_effect = lambda arg: project_path if str(arg) == ".auto-pr.env" else user_path
                with patch("auto_pr.config_cli.Path.cwd", return_value=Path(tmpdir)):
                    result = runner.invoke(config, ["show"])
                    assert result.exit_code == 0
...
def test_config_unset_no_file():
    """Test unset command when .auto-pr.env doesn't exist (lines 90-91)."""
    runner = CliRunner()
    with tempfile.TemporaryDirectory() as tmpdir:
        fake_path = Path(tmpdir) / ".auto-pr.env"
...
def test_config_show_no_configs_exist():
    ...
    runner = CliRunner()
    with tempfile.TemporaryDirectory() as tmpdir:
        fake_user_path = Path(tmpdir) / ".auto-pr.env"

        # Change to the temp directory so project .auto-pr.env is also in tmpdir and doesn't exist
        with runner.isolated_filesystem(temp_dir=tmpdir):
            with patch("auto_pr.config_cli.AUTO_PR_ENV_PATH", fake_user_path):
                result = runner.invoke(config, ["show"])
                assert result.exit_code == 0
                assert "No $HOME/.auto-pr.env found" in result.output
                assert "No project-level .auto-pr.env found" in result.output
...
def test_config_show_with_none_values():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        fake_user_path = Path(tmpdir) / ".auto-pr.env"
        fake_project_path = Path(tmpdir) / ".auto-pr.env"
