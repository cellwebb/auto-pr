...
class TestClaudeCodeImports:
    ...
    def test_import_provider(self):
        ...
        from auto_pr.providers import claude_code  # noqa: F401
...
class TestClaudeCodeAPIKeyValidation:
    ...
    def test_missing_access_token_error(self):
        ...
        with patch("auto_pr.providers.claude_code.load_stored_token", return_value=None):
            with pytest.raises(AIError) as exc_info:
                call_claude_code_api("claude-haiku-4-5", [], 0.7, 1000)

            assert exc_info.value.error_type == "authentication"
            assert "auto-pr auth claude-code login" in str(exc_info.value)
...
class TestClaudeCodeProviderMocked(BaseProviderTest):
    ...
    def provider_module(self) -> str:
        return "auto_pr.providers.claude_code"
    ...
    def auth_context(self):
        ...
        return patch("auto_pr.providers.claude_code.load_stored_token", return_value="test-token")
...
class TestClaudeCodeEdgeCases:
    ...
    def test_claude_code_missing_content(self):
        ...
        with patch("auto_pr.providers.claude_code.load_stored_token", return_value="test-token"):
            with patch("auto_pr.providers.base.httpx.post") as mock_post:
    ...
    def test_claude_code_authentication_header(self):
        ...
        with patch("auto_pr.providers.claude_code.load_stored_token", return_value="test-token-12345"):
            with patch("auto_pr.providers.base.httpx.post") as mock_post:
                mock_response = MagicMock()
                mock_response.json.return_value = {"content": [{"text": "test response"}]}
                mock_response.raise_for_status = MagicMock()
    ...
    def test_claude_code_401_error_message(self):
        ...
        with patch("auto_pr.providers.claude_code.load_stored_token", return_value="expired-token"):
            with patch("auto_pr.providers.base.httpx.post") as mock_post:
                mock_response = MagicMock()
...
class TestClaudeCodeIntegration:
    ...
    def test_real_api_call(self):
        ...
        from auto_pr.oauth.claude_code import load_stored_token

        if not load_stored_token():
            pytest.skip("Claude Code OAuth token not found - run 'auto-pr auth claude-code login' first")
