...
from auto_pr.init_cli import _configure_language, _load_existing_env, _prompt_required_text, init
...
def test_init_cli_load_existing_env_new_file(tmp_path):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        env_path = Path(tmpdir) / ".auto-pr.env"
        with mock.patch("auto_pr.init_cli.AUTO_PR_ENV_PATH", env_path), mock.patch("click.echo") as mecho:
            result = _load_existing_env()
            assert result == {}
            assert env_path.exists()
            assert "Created $HOME/.auto-pr.env" in str(mecho.call_args)
...
def test_init_cli_configure_language_success(tmp_path):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        with (
            mock.patch("auto_pr.init_cli.AUTO_PR_ENV_PATH", env_path),
            mock.patch("auto_pr.init_cli.configure_language_init_workflow") as mconfig,
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        with _patch_env_paths(env_path):
            with (
                ...
                assert "AUTO_PR_MODEL='openai:gpt-4'" in env_text
                assert "OPENAI_API_KEY='openai-key'" in env_text
                assert "AUTO_PR_LANGUAGE='English'" in env_text
                assert "AUTO_PR_TRANSLATE_PREFIXES='false'" in env_text
                assert "Auto-PR environment setup complete" in result.output
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        with _patch_env_paths(env_path):
            with (
                ...
                assert result.exit_code == 0
                env_text = env_path.read_text()
                assert "AUTO_PR_LANGUAGE='Spanish'" in env_text
                assert "AUTO_PR_TRANSLATE_PREFIXES='true'" in env_text
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        with _patch_env_paths(env_path):
            with (
                ...
                assert "AUTO_PR_MODEL='openai:gpt-5'" in env_text
