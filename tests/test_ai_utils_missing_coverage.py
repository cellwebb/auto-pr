...
from auto_pr.ai_utils import generate_with_retries
...
class TestAIUtilsMissingCoverage:
    ...
    @patch("auto_pr.ai_utils.TokenStore")
    @patch("auto_pr.ai_utils.refresh_token_if_expired")
    def test_claude_code_token_refresh_failure(self, mock_refresh, mock_token_store):
        ...
        assert "Please authenticate with 'auto-pr auth claude-code login'" in str(exc_info.value)

    @patch("auto_pr.ai_utils.TokenStore")
    @patch("auto_pr.ai_utils.refresh_token_if_expired", return_value=True)
    ...
    @patch("auto_pr.ai_utils.QwenOAuthProvider")
    ...
    def test_qwen_auth_flow_connection_error(self, mock_oauth_provider):
        ...
        assert "Run 'auto-pr auth qwen login' to authenticate manually" in str(exc_info.value)
