...
def _setup_env_file(tmpdir: str) -> Path:
    env_path = Path(tmpdir) / ".auto-pr.env"
    env_path.touch()
    return env_path
...
def _patch_env_paths(env_path: Path):
    """Patch all AUTO_PR_ENV_PATH locations since init_cli imports from model_cli and language_cli."""
    with (
        mock.patch("auto_pr.init_cli.AUTO_PR_ENV_PATH", env_path),
        mock.patch("auto_pr.model_cli.AUTO_PR_ENV_PATH", env_path),
        mock.patch("auto_pr.language_cli.AUTO_PR_ENV_PATH", env_path),
    ):
        yield
...
def test_should_show_rtl_warning_for_init_no_config() -> None:
    ...
    with mock.patch("auto_pr.model_cli.AUTO_PR_ENV_PATH") as mock_path:
...
def test_should_show_rtl_warning_for_init_not_confirmed() -> None:
    ...
    with (
        mock.patch("auto_pr.model_cli.AUTO_PR_ENV_PATH") as mock_path,
        mock.patch("auto_pr.model_cli.load_dotenv"),
        mock.patch("auto_pr.model_cli.os.getenv", return_value="false"),
    ):
...
def test_should_show_rtl_warning_for_init_already_confirmed() -> None:
    ...
    with (
        mock.patch("auto_pr.model_cli.AUTO_PR_ENV_PATH") as mock_path,
        mock.patch("auto_pr.model_cli.load_dotenv"),
        mock.patch("auto_pr.model_cli.os.getenv", return_value="true"),
    ):
        mock_path.exists.return_value = True
...
def test_show_rtl_warning_for_init_proceeds() -> None:
    ...
    with (
        mock.patch("auto_pr.model_cli.click.echo") as mock_echo,
        mock.patch("auto_pr.model_cli.click.style") as mock_style,
        mock.patch("auto_pr.model_cli.questionary.confirm") as mock_confirm,
        mock.patch("auto_pr.model_cli.set_key") as mock_set_key,
    ):
...
def test_prompt_required_text_success() -> None:
    ...
    with mock.patch("auto_pr.model_cli.questionary.text") as mock_text:
...
def test_prompt_required_text_empty_then_valid() -> None:
    ...
    with (
        mock.patch("auto_pr.model_cli.questionary.text") as mock_text,
        mock.patch("auto_pr.model_cli.click.echo") as mock_echo,
    ):
...
def test_load_existing_env_file_exists() -> None:
    ...
    with (
        mock.patch("auto_pr.model_cli.AUTO_PR_ENV_PATH", mock_path),
        mock.patch("auto_pr.model_cli.click.echo") as mock_echo,
        mock.patch("auto_pr.model_cli.dotenv_values", return_value={"EXISTING": "value", "EMPTY": None}),
    ):
...
def test_load_existing_env_file_new() -> None:
    ...
    with (
        mock.patch("auto_pr.model_cli.AUTO_PR_ENV_PATH", mock_path),
        mock.patch("auto_pr.model_cli.click.echo") as mock_echo,
        mock.patch("auto_pr.model_cli.dotenv_values", return_value={}),
    ):
        result = _load_existing_env()
...
def test_model_cli_basic(tmp_path):
    ...
    env_path = tmp_path / ".auto-pr.env"
    env_path.touch()

    # Mock both AUTO_PR_ENV_PATH locations since init_cli imports from model_cli
    with mock.patch("auto_pr.model_cli.AUTO_PR_ENV_PATH", env_path):
        with mock.patch("auto_pr.init_cli.AUTO_PR_ENV_PATH", env_path):  # For init_cli's _load_existing_env
            with (
