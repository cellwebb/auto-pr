...
def mock_run_subprocess():
    """Mock for auto_pr.git.run_subprocess."""
    with patch("auto_pr.git.run_subprocess") as mock:
        yield mock


@pytest.fixture
def mock_get_staged_files():
    """Mock for auto_pr.git.get_staged_files."""
    with patch("auto_pr.git.get_staged_files") as mock:
...
@pytest.fixture
def mock_get_config():
    """Mock for auto_pr.config.get_config."""
    with patch("auto_pr.config.get_config") as mock:
        mock.return_value = {
...
def mock_commit_changes():
    """Mock for auto_pr.git.commit_changes."""
    with patch("auto_pr.git.commit_changes") as mock:
        yield mock
...
@pytest.fixture
def mock_logging():
    """Mock for auto_pr.git.logging."""
    with patch("auto_pr.git.logging") as mock:
...
def mock_count_tokens():
    """Mock for auto_pr.ai.count_tokens."""
    with patch("auto_pr.ai.count_tokens") as mock:
...
def mock_build_prompt():
    """Mock for auto_pr.prompt.build_prompt."""
    with patch("auto_pr.prompt.build_prompt") as mock:
...
def clean_env_state():
    ...
    patterns_to_clear = [
        "AUTO_PR_",  # Auto-PR configuration
...
def mock_stage_files():
    """Mock for auto_pr.git.stage_files."""
    with patch("auto_pr.git.stage_files") as mock:
        yield mock


@pytest.fixture
def mock_chat():
    """Mock for auto_pr.ai.chat."""
    with patch("auto_pr.ai.chat") as mock:
