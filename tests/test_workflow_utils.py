...
from auto_pr.workflow_utils import (
    check_token_warning,
...
def test_execute_commit_uses_hook_timeout():
    with (
        patch("auto_pr.git.run_git_command") as mock_git,
        patch("auto_pr.workflow_utils.logger.info"),
        patch("auto_pr.workflow_utils.console.print"),
    ):
...
def test_restore_staging():
    ...
    with patch("auto_pr.git.run_git_command") as mock_git:
...
def test_restore_staging_reapplies_diff(tmp_path):
    ...
    with patch("auto_pr.git.run_git_command", side_effect=fake_run_git):
...
def test_restore_staging_handles_errors():
    ...
    with (
        patch("auto_pr.git.run_git_command", side_effect=git_side_effect) as mock_git,
        patch("auto_pr.workflow_utils.logger.warning") as mock_logger,
    ):
        restore_staging(files)
...
class TestCollectInteractiveAnswers:
    ...
    @patch("auto_pr.workflow_utils.console.print")
    @patch("auto_pr.workflow_utils.prompt")
