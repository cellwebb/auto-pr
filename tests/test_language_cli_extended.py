...
from auto_pr.language_cli import (
    center_text,
    get_terminal_width,
    is_rtl_text,
    language,
...
def test_should_show_rtl_warning_no_config():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        with patch("auto_pr.language_cli.AUTO_PR_ENV_PATH", fake_path):
            ...
            if "AUTO_PR_RTL_CONFIRMED" in os.environ:
                del os.environ["AUTO_PR_RTL_CONFIRMED"]
...
def test_should_show_rtl_warning_config_exists_confirmed_true():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        fake_path.write_text("AUTO_PR_RTL_CONFIRMED=true\n")
...
def test_should_show_rtl_warning_config_exists_confirmed_variants():
    ...
    for value, expected in test_values.items():
        with tempfile.TemporaryDirectory() as tmpdir:
            fake_path = Path(tmpdir) / ".auto-pr.env"
            fake_path.write_text(f"AUTO_PR_RTL_CONFIRMED={value}\n")
            with patch("auto_pr.language_cli.AUTO_PR_ENV_PATH", fake_path):
                # Clear any existing environment variable first
                if "AUTO_PR_RTL_CONFIRMED" in os.environ:
                    del os.environ["AUTO_PR_RTL_CONFIRMED"]
...
def test_show_rtl_warning_proceed_true():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        with (
            ...
            patch("auto_pr.language_cli.get_terminal_width", return_value=80),
            patch("auto_pr.language_cli.center_text", return_value="  ⚠️  RTL Language Detected  "),
            ...
            assert "AUTO_PR_RTL_CONFIRMED='true'" in content
